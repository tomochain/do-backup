...
@click.option(
    "--only",
    type=click.Choice(["snapshot", "prune"]),
    help="Only snapshot or only prune",
)
@click.option("--debug", is_flag=True, help="Enable debug logging")
@click.version_option(version=__version__)
def entrypoint(
    config: click.File, do_token: str, only: str, debug: bool
) -> None:
...
def _load_config(config: click.File) -> Dict[str, Dict]:
    ...
    try:
        ...
        log.critical(
            "Could not parse toml in config from {}: {}".format(config.name, e)
        )
        sys.exit(1)
...
def _process_droplets(
    conf: Dict[str, Union[Dict[str, str], str]], only: str
) -> None:
...
def _snapshot_droplet(droplet: digitalocean.Droplet) -> None:
    ...
    global error
    name = "goutte-{}-{}-{}".format(
        droplet.name, date.today().strftime("%Y%m%d"), uuid.uuid4().hex[:5]
    )
    try:
...
def _prune_droplet_snapshots(
    droplet: digitalocean.Droplet, retention: int
) -> None:
    ...
    try:
        ...
        if len(snapshots) > retention:
            log.debug(
                f"{droplet.name} - Exceed retention policy by "
...
def _snapshot_volume(volume: digitalocean.Volume) -> None:
    ...
    global error
    name = "goutte-{}-{}-{}".format(
        volume.name, date.today().strftime("%Y%m%d"), uuid.uuid4().hex[:5]
    )
    try:
...
def _prune_volume_snapshots(
    volume: digitalocean.Volume, retention: int
) -> None:
    """Prune goutte snapshots if tmore than the configured retention time"""
    global error
    try:
        ...
        if len(snapshots) > retention:
            log.debug(
                f"{volume.name} - Exceed retention policy by "
                f"{len(snapshots) - retention}"
            )
            for snapshot in snapshots[: len(snapshots) - retention]:
...
def _order_snapshots(
    snapshots: List[digitalocean.Snapshot],
